...
<?foreach($arResult["ITEMS"] as $arItem):?>
	...
	?>
	<div class="nwes_events" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
		<div class="wrapp_img">
		...
		<?endif?>
                <p><?=$arItem['PROPERTIES'][27]['VALUE']?></p>
		<?/*if($arParams["DISPLAY_NAME"]!="N" && $arItem["NAME"]):?>
				<p class="news-title"><?echo $arItem["NAME"]?></p>
		<?endif;*/?>
				</div>
				</div>
		<?if($arParams["DISPLAY_PREVIEW_TEXT"]!="N" && $arItem["PREVIEW_TEXT"]):?>
			<a href="<?=$arItem["DETAIL_PAGE_URL"]?>"><div class="text_nwes"><?echo $arItem["PREVIEW_TEXT"];?></div></a>
		<?endif;?>
		<?if($arParams["DISPLAY_PICTURE"]!="N" && is_array($arItem["PREVIEW_PICTURE"])):?>
